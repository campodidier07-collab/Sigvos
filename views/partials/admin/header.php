    <header class="bg-white/80 backdrop-blur-md border-b border-[#d8eee4] sticky top-0 z-40 flex items-center justify-between px-6 h-16" style="box-shadow:0 1px 12px rgba(11,93,70,.05);">
        <div class="flex items-center gap-3">
            <div id="header-icon" class="w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs" style="background:linear-gradient(135deg,#0b6b4f,#18b57a);">
                <i class="fas fa-house"></i>
            </div>
            <div>
                <h1 id="header-title" class="text-base font-bold text-[#16332b]">Panel</h1>
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="relative">
                <button onclick="document.getElementById('panel-notif').classList.toggle('hidden')"
                    ...
                    <?php if ($notifs_no_leidas > 0): ?>
                    <span class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center"><?= $notifs_no_leidas ?></span>
                    <?php endif; ?>
                </button>
                <!-- Panel desplegable -->
                <div id="panel-notif" class="hidden absolute right-0 top-11 w-[480px] bg-white rounded-2xl shadow-2xl border border-[#d8eee4] z-50 overflow-hidden">
                    <div class="flex items-center justify-between px-6 py-4 border-b border-[#d8eee4] bg-[#f0fdf4]">
                        ...
                        <a href="../../controllers/NotificacionController.php?accion=marcar_todas"
                            class="text-sm text-[#0f8f67] hover:underline font-semibold">Marcar todas</a>
                        <?php endif; ?>
                    </div>
                    <div class="max-h-96 overflow-y-auto divide-y divide-[#f0fdf4]">
                        ...
                        <div class="flex items-start gap-4 px-6 py-4 <?= $n['leida'] ? 'opacity-50' : 'bg-white' ?> hover:bg-slate-50 transition">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-slate-400 mt-1 line-clamp-2"><?= htmlspecialchars($n['mensaje']) ?></p>
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xs text-slate-400 mt-0.5">
                                    <?= $ultimaSyncFecha ? 'Última: ' . date('d/m/Y H:i', strtotime($ultimaSyncFecha)) : 'Sin sincronizar' ?>
                                </p>
                            </div>
